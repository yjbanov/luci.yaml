...
class Scheduler {
  ...
  void _scheduleOnAgent(WorkspaceTarget target) async {
    ...
    final AgentPool pool = agentPools[target.agentProfile];
    if (pool == null) {
      throw StateError(
        'Target ${target.name} requested unknown agent profile: ${target.agentProfile}',
      );
    }
    ...
    try {
      await agent.runTarget(target);
      _runningTargets.remove(target);
      print('Finished ${target.name}');
      _completedTargets.add(target.name);
      _scheduleAvailableTargets();
    } catch (error) {
      _buildCompleter.completeError('Target ${target.name} failed');
      rethrow;
    }
  ...
  bool _hasDependenciesSatisfied(WorkspaceTarget target) {
    return _completedTargets.containsAll(target.dependencies.map((t) => t.name));
  }
}
...
class Agent {
  ...
  Future<void> runTarget(WorkspaceTarget target) async {
